...
public final class QueryUtils {
    ...
    public static List<News> fetchNewsData(String requestUrl) {
        ...
        // Extract relevant fields from the JSON response and create a list ofarticles
        List<News> newsList = extractresultsFromJson(jsonResponse);
    ...
    private static List<News> extractresultsFromJson(String newsJSON) {
        ...
        try {
            ...
            // For each news article in the resultsJSONArray, create an {@link News} object
            for (int i = 0; i < resultsJSONArray.length(); i++) {

                // Get a single Article at position i within the list of newsList
                JSONObject currentNewsArticle = resultsJSONArray.getJSONObject(i);
